...
import { Injectable } from '@nestjs/common';
import { DeepSeek } from "langchain/llms/deepseek"; // Import DeepSeek

@Injectable()
export class AiService {
  private llm: DeepSeek; // Store the LLM instance

  constructor() {
    this.llm = new DeepSeek({  // Initialize with your API key
      openAIApiKey: process.env.DEEPSEEK_API_KEY,  // From environment variables!
    });
  }
